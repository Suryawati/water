...
from water.models import AirValve, MeterInduk
...
# map
    ...
    path('<lati>/<long>/<int:meter>/premise.geojson', assets.PremiseLayer.as_view(
        model=Premise,
        properties=('id', 'name', 'idpel', 'address1', 'address2', 'kode_lokasi', 'premise_type', 'status', 'kode_gol', 'location_geometry')
    ), name='json-premise'),

    path('<lati>/<long>/<int:meter>/pump.geojson', assets.PumpLayer.as_view(
        model=Pump,
        properties=('id_pompa', 'location_geometry')
    ), name='json-pump'),

    path('<lati>/<long>/<int:meter>/pipa.geojson', assets.PipaLayer.as_view(
        model=Pipa,
        properties=('id_pipa', 'pemilik', 'kelas_pipa', 'location_geometry', 'status', 'diameter_pipa', 'material_pipa', 'pjg_pipa_geometris')
    ), name='json-pipa'),
